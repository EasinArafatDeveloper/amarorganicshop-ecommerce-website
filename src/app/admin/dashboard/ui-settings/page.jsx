...
import { Save, Image as ImageIcon, Phone, Mail, Globe, LayoutTemplate, ToggleLeft, ToggleRight, LayoutDashboard, Loader2, Truck, Plus, Trash2, Tags, MessageSquareQuote, ShieldCheck } from 'lucide-react';
...
export default function UISettingsPage() {
    const [settings, setSettings] = useState({
        logoUrl: '',
        heroBannerUrl: '',
        heroBanners: [], // New state for multiple banners
        ...
        try {
            ...
            if (res.ok && data) {
                setSettings(prev => ({
                    ...
                    heroSliderAutoplay: data.heroSliderAutoplay || 5000
                }));
            }
        } catch (error) {
    ...
    const handleTrustBadgeChange = (e) => {
        const { name, value } = e.target;
        setSettings(prev => ({
            ...prev,
            trustBadges: {
    ...
    const handleAddBanner = () => {
        setSettings(prev => ({
            ...prev,
            heroBanners: [...(prev.heroBanners || []), { url: '', link: '/shop' }]
        }));
    };
    ...
    const handleSave = async (e) => {
        ...
        // Final check to ensure all empty/incomplete banners are filtered or handled
        const cleanedBanners = (settings.heroBanners || []).filter(b => b.url.trim() !== '');
        const payload = { ...settings, heroBanners: cleanedBanners };
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Refresh local state with cleaned data if necessary
                setSettings(prev => ({ ...prev, heroBanners: cleanedBanners }));
            } else {
            ...
            <form onSubmit={handleSave} className="space-y-6">
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Hero Slider Management (Merged) */}
                                <div className="border-t border-gray-100 pt-6">
                                    <div className="flex justify-between items-center mb-4">
                                        <div>
                                            <label className="block text-sm font-bold text-gray-700">Hero Section Banners (Slider)</label>
                                            <p className="text-[10px] text-gray-400 font-normal">Add multiple banners (1600x749 recommended for best fit)</p>
                                        </div>
                                        <div className="flex flex-col md:flex-row items-end md:items-center gap-3">
                                            <div className="flex items-center gap-2">
                                                <label className="text-xs font-bold text-gray-500">Autoplay:</label>
                                                <select
                                                    name="heroSliderAutoplay"
                                                    value={settings.heroSliderAutoplay}
                                                    onChange={(e) => setSettings(prev => ({ ...prev, heroSliderAutoplay: Number(e.target.value) }))}
                                                    className="px-2 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs font-bold text-gray-600 outline-none focus:ring-2 focus:ring-primary/20"
                                                >
                                    ...
                                    <div className="space-y-4">
                                        {(!settings.heroBanners || settings.heroBanners.length === 0) ? (
                                            <div className="text-center py-10 border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50/50">
                                                <div className="bg-white p-3 rounded-full w-fit mx-auto shadow-sm border border-gray-100 mb-3">
                                                    <ImageIcon size={24} className="text-gray-300" />
                                                </div>
                                                <p className="text-gray-400 text-sm font-medium">No banners added yet.</p>
                                                <button type="button" onClick={handleAddBanner} className="mt-2 text-primary text-xs font-bold hover:underline">Add your first banner</button>
                                            </div>
                                        ) : (
                                            <div className="grid grid-cols-1 gap-4">
                                                {settings.heroBanners.map((banner, index) => (
                                                    <div key={index} className="p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-4 transition-all hover:border-primary/20">
                                                        <div className="flex justify-between items-center">
                                                            <div className="flex items-center gap-2">
                                                                <span className="flex items-center justify-center w-6 h-6 bg-primary text-white text-[10px] font-black rounded-full shadow-sm shadow-primary/20">
                                                                    {index + 1}
                                                                </span>
                                                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Banner Slide</span>
                                                            </div>
                                                            <button 
                                                                type="button" 
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                            <div className="space-y-1.5">
                                                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Banner Image URL</label>
                                                                <div className="relative">
                                                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                                                                        <ImageIcon size={14} />
                                                                    </div>
                                                                    <input 
                                                                        ...
                                                                        placeholder="https://..." 
                                                                        className="w-full pl-9 pr-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/10 outline-none"
                                                                    />
                                                                </div>
                                                            </div>
                                                            <div className="space-y-1.5">
                                                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Target Link / URL</label>
                                                                <div className="relative">
                                                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                                                                        <Globe size={14} />
                                                                    </div>
                                                                    <input 
                                                                        ...
                                                                        placeholder="/shop or product link..." 
                                                                        className="w-full pl-9 pr-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/10 outline-none"
                                                                    />
                                                                </div>
                                                            </div>
                                                        </div>
                                                        {banner.url && banner.url.startsWith('http') && (
                                                            <div className="relative h-32 md:h-40 w-full rounded-xl border-2 border-white bg-white overflow-hidden shadow-sm group">
                                                                <img src={banner.url} alt={`Banner ${index+1}`} className="w-full h-full object-cover rounded-lg group-hover:scale-105 transition-transform duration-500" />
                ...
                {activeTab === 'contact' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        ...
                        {/* Trust Badges Details Page */}
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                            <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2">
                                <ShieldCheck className="w-5 h-5 text-green-600" />
                                Details Page Trust Badges
                            </h3>
                            <div className="space-y-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1.5">Easy Returns Subtitle</label>
                                    <input type="text" name="returnsText" value={settings.trustBadges.returnsText} onChange={handleTrustBadgeChange} placeholder="e.g. 7 Days return policy" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" />
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Secure Payment Subtitle</label>
                                        <input type="text" name="paymentText" value={settings.trustBadges.paymentText} onChange={handleTrustBadgeChange} placeholder="e.g. 100% Secure checkout" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1.5">Quality Assured Subtitle</label>
                                        <input type="text" name="qualityText" value={settings.trustBadges.qualityText} onChange={handleTrustBadgeChange} placeholder="e.g. 100% Authentic products" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" />
                                    </div>
                                </div>
                                <p className="text-xs text-gray-400">Note: The "Free Delivery" text is automatically synced with the settings in the Delivery Zones tab.</p>
                            </div>
                        </div>
                    </div>
                )}
