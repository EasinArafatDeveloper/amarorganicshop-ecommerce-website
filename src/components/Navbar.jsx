                ...
                <div className={`fixed top-0 right-0 h-full w-full sm:w-[350px] bg-white z-[70] transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[60vh] p-6 space-y-4">
                            {cartItems.map((item) => (
                                <div key={item.cartItemId || item.id} className="bg-white p-3 rounded-xl border border-gray-100 flex gap-3 relative shadow-sm">
                                    ...
                                    <div className="flex-1 flex flex-col">
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            ...
                                            {/* Quantity Control */}
                                            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-md p-0.5">
                                                <button onClick={() => updateQuantity(item.cartItemId || item.id, item.quantity - 1)} className="w-6 h-6 flex items-center justify-center bg-white rounded text-gray-600 hover:text-secondary">
                                                    <Minus size={14} />
                                                </button>
                                    ...
                                    <button onClick={() => removeFromCart(item.cartItemId || item.id)} className="absolute top-3 right-3 text-gray-400 hover:text-red-500 transition-colors">
                                        <X size={18} />
                                    </button>
                                </div>
                            ))}
